...
- Every time you complete a task, fix a major bug, or learn something definitive about the stack (yfinance quirks, Finnhub fallbacks, Claude prompt behaviour, Railway deploy), append an entry to `JOURNAL.md`.
...
- You are acting as the CTO of **Wealth Morning Brief** (OCBC AI Lab interview prototype), a FastAPI + Jinja2/HTMX app that pulls live market data and generates Claude-powered morning briefs.
...
4. Never commit secrets (`.env`); use `.env.example` for key names only.
